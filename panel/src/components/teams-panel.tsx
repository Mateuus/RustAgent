 ...
 *
 * A lista responde "quem está com quem AGORA" e muda a cada convite
 * aceito; a configuração é uma decisão que se toma uma vez e vale
 * para o servidor inteiro. Elas moravam juntas, e a menos usada
 * ocupava o topo da mais usada.
 *
 * Pílulas, e não a barra sublinhada das abas do servidor: dois
 * níveis com o mesmo desenho fazem a pessoa perder de vista em qual
 * deles está. É o mesmo desenho da tela de Configurações.
 */
type View = 'lista' | 'config';

const VIEWS: readonly { readonly key: View; readonly label: string }[] = [
  { key: 'lista', label: 'Lista de equipes' },
      ...
      <div className="flex flex-wrap items-stretch border border-border bg-surface">
        {VIEWS.map((item, index) => (
          <div key={item.key} className="flex items-stretch">
            {/* A divisória: sem ela, dois rótulos em maiúsculas viram
                uma faixa contínua de texto. */}
            ...
            >
              {item.label}
              {item.key === 'lista' && teams.length > 0 && (
                <span className="ml-1 font-normal text-muted">({teams.length})</span>
              )}
            </button>
          </div>
        ))}
...
function TeamSettingsCard({ serverId }: { readonly serverId: string }) {
  ...
  if (saved === null) {
    return (
      <StateBlock
        variant="error"
        title="Não consegui ler a configuração"
        detail="O agente não respondeu. Com o servidor parado, o valor que vale continua sendo o que está gravado aqui."
      />
    );
  }
    ...
    <section className="border border-border bg-surface">
      ...
      <div className="p-3">
        ...
        <p className="mt-2 max-w-2xl text-2xs text-muted">
          É o <span className="font-mono">relationshipmanager.maxteamsize</span> do Rust, e vale na
          hora — inclusive para as equipes que já existem. Zero{' '}
          <strong className="text-foreground">desliga</strong> equipes no servidor.
        </p>

        <p className="mt-1 max-w-2xl text-2xs text-muted">
          O jogo não guarda este valor: ele volta a 8 em todo restart. Quem tem a memória dele é o
          agente, que o reaplica quando o RCON conecta.
        </p>
      </div>
    </section>
  );
