...
export function TeamsPanel({ serverId }: TeamsPanelProps) {
  ...
  /** Qual das duas metades da tela está aberta. */
  ...
  const shown = useMemo(
    () => filterAndSort(teams ?? [], query, order),
    [teams, query, order],
  );
  ...
  if (teams === null)
    return <StateBlock variant="loading" title="Lendo as equipes…" />;

  const online = teams.reduce(
    (total, team) =>
      total + team.members.filter((member) => member.online).length,
    0,
  );

  return (
    <div className="space-y-3">
      {/* ####  DUAS COISAS, DUAS ABAS  ####

          A lista responde "quem está com quem AGORA" e muda a cada
          convite aceito; a configuração é uma decisão que se toma uma
          vez e vale para o servidor inteiro.

          Elas moravam na mesma tela, e a configuração — que é a
          menos usada — ficava ocupando o topo da mais usada. */}
      <div className="flex border-b border-border">
        <SubTab active={view === "lista"} onClick={() => setView("lista")}>
          Lista de equipes {teams.length > 0 && `(${String(teams.length)})`}
        </SubTab>
        <SubTab active={view === "config"} onClick={() => setView("config")}>
          Configurações
        </SubTab>
      </div>

      {view === "config" && <TeamSettingsCard serverId={serverId} />}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 className="flex items-center gap-2 font-condensed text-sm font-bold uppercase tracking-wide">
                <span
                  aria-hidden="true"
                  className="h-4 w-[3px] shrink-0 bg-rust"
                />
                Equipes no jogo
                <span className="font-normal text-muted">({teams.length})</span>
              </h2>
              <p className="mt-1 text-2xs text-muted">
                Lido do servidor agora. O time é do jogo — o que a casa
                acrescenta é o <strong className="text-foreground">nome</strong>{" "}
                e o <strong className="text-foreground">cargo</strong>.
                {maxSize > 0 && ` Cabem ${String(maxSize)}.`}
                {online > 0 &&
                  ` ${String(online)} jogador(es) online em equipe.`}
              </p>
            </div>
    ...
    <section className="border border-border bg-surface">
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border p-3">
        <div className="min-w-0">
          {editing ? (
            ...
            >
              ...
              <Pencil
                aria-hidden="true"
                className="h-4 w-4 text-muted group-hover:text-rust"
              />
            </button>
          )}
        ...
        <ConfirmButton
          ...
          onConfirm={() =>
            void onAct(`disband:${team.teamId}`, () =>
              agent.disbandTeam(serverId, team.teamId),
            )
          }
        />
      </header>
          ...
          (team) =>
            ...
            team.members.some(
              (member) =>
                ...
                member.steamId.includes(needle),
            ),
        );
  ...
  async function save(): Promise<void> {
    ...
    if (!Number.isInteger(value) || value < 0 || value > 64) {
      toast.error("Valor inválido", {
        description: "Use um número inteiro entre 0 e 64.",
      });
      return;
    }
    ...
    <section className="border border-border bg-surface">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-3 py-2">
        ...
        <span className="flex items-center gap-2 text-2xs text-muted">
          ...
          {/* ####  A DIVERGÊNCIA É O ALERTA  ####

              Ela aparece quando o jogo está com outro valor — e isso
              não é bug: o convar não persiste, e volta a 8 em todo
              restart. Ver o cabeçalho do componente. */}
          {diverge && (
      ...
      <div className="p-3">
        ...
        <p className="mt-2 max-w-2xl text-2xs text-muted">
          É o <span className="font-mono">relationshipmanager.maxteamsize</span>{" "}
          do Rust, e vale na hora — inclusive para as equipes que já existem.
          Zero <strong className="text-foreground">desliga</strong> equipes no
          servidor.
        </p>

        <p className="mt-1 max-w-2xl text-2xs text-muted">
          O jogo não guarda este valor: ele volta a 8 em todo restart. Quem tem
          a memória dele é o agente, que o reaplica quando o RCON conecta.
        </p>
      </div>
    </section>
  );
